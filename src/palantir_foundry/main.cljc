(ns palantir_foundry.main
  "Kotodama WASM entrypoint for the Palantir_foundry clean-room actor (L4) — Clojure port.
  ...
  facts (`palantir_foundry.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "palantir_foundry")
(def tier "L4")
...
(def entity-specs
  [{:entity "Case"      :plural "cases"      :id-prefix "palantir_cas"
    :fields [:number :jurisdiction :status :filedAt]           :required [:number :jurisdiction]
    :coerce {}                                :refs {}}
   {:entity "Party"     :plural "parties"    :id-prefix "palantir_par"
    :fields [:caseId :role :name]                              :required [:role :name]
    :coerce {}                                :refs {:caseId "Case"}}
   {:entity "Document"  :plural "documents"  :id-prefix "palantir_doc"
    :fields [:caseId :type :contentRef]                        :required [:type]
    :coerce {}                                :refs {:caseId "Case"}}
   {:entity "Incident"  :plural "incidents"  :id-prefix "palantir_inc"
    :fields [:type :jurisdiction :occurredAt]                  :required [:type :jurisdiction]
    :coerce {}                                :refs {}}
   {:entity "Watchlist" :plural "watchlists" :id-prefix "palantir_wat"
    :fields [:name :program :entries]                          :required [:name :program]
    :coerce {:entries :int}                   :refs {}}
   {:entity "Sanction"  :plural "sanctions"  :id-prefix "palantir_san"
...
(defn emit-facts
  "EAVT facts for one record: {\"palantir_foundry.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "palantir_foundry-compat" :tier tier :entities entities} 200])
...
(defn start! [] :palantir_foundry-compat/ready)
